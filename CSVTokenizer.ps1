...
function TableSelection(){
	...
	while ($Index -in 0..$ListMaxIndex){
		...
		switch ($Reply) {
			...
			'H' {HelpPage; Break}
			'S' {Summary($HeaderSelection); Read-Host "ENTER TO CONTINUE..."; Break}
...
function TokenGenerator(){
	param($InputCSVFile, $SelectedColumnsLength, $FileDelimiter)
	$FileReaderLineCount = New-Object Microsoft.VisualBasic.FileIO.TextFieldParser $InputCSVFile
	$FileReaderLineCount.TextFieldType = [Microsoft.VisualBasic.FileIO.FieldType]::Delimited
	$FileReaderLineCount.SetDelimiters($FileDelimiter)
	$FileReaderLineCount.HasFieldsEnclosedInQuotes = $true
	$InputFileLineCount = 0
	$ColumnTitleLine = $True
	while ($FileReaderLineCount.ReadLine() -ne $null) {
		$InputFileLineCount++
	}
	$FileReaderLineCount.Close()
	$LowerBound = 1
	$UpperBound = (($SelectedColumnsLength)*($InputFileLineCount))
	$ThisTokenList = ($LowerBound..$UpperBound | ForEach-Object { "T[{0:X}]" -f $_ })
	Return $ThisTokenList
}
function FileTokenizer(){
	param($InputFilePath, $ColumnTitleData, $ColumnsToMask, $DataTokens, $CSVDelimiter)
		...
		foreach ($Column in $ColumnsToMask) {
			if ($Column -lt $RowValues.Length) {
				...
				if(!($TokenKeyData.ContainsKey($OriginalValue))){
					$TokenKeyData[$OriginalValue] = $DataTokens[$TokenCounter]
					$TokenCounter++
				}
...
if ($args[0]){
	...
	$SelectedColumns = TableSelection(@())
}
